...
public:
  ...
  void i_sub_arrel(node_arbre* primer, node_arbre* asub, const T& x, bool añadir, bool& encontrado) {
    if (primer->info == x and not encontrado) {
      encontrado = true;
      añadir = true; 
    }
    if (añadir) {
      asub->info = primer->info;
    }
    i_sub_arrel(primer->segE, asub->segE, x, añadir, encontrado);
    i_sub_arrel(primer->segD, asub->segD, x, añadir, encontrado); 
    }
 ...
 /* Post: si A conte x, asub es el subarbre d'A resultat de la cerca;
  ...
  
    {
      ...
      i_sub_arrel(primer_node, asub.primer_node, x, false, encontrado);
    }
