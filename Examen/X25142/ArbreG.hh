...
public:
  ...
  void i_max_suma_cami(node_arbreGen* node, T suma_act, T& suma_max) {
    if (node == NULL) {
      if (suma_act > suma_max) suma_max = suma_act;
      return; 
    }
    suma_act += node->info;
    int size = node->seg.size(); 
    for (int i = 0; i < size; ++i) {
      i_max_suma_cami(node->seg[i], suma_act, suma_max);
    }
  }
 ...
 /* Post: el resultat es la suma del cami de suma maxima del parametre implicit */
 {
    T suma_max;
    T suma_act;
    i_max_suma_cami(primer_node, suma_act, suma_max)
    return suma_max;
 }
